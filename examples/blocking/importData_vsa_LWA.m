...
ifY = lat >= Domain.yLim( 1 ) & lat <= Domain.yLim( 2 );
nX2 = nnz( ifX );
nY2 = nnz( ifY );
...
% nG is the number of longitude gridpoints where we extract "slices"
nG = nX2;

% Output data dimension (equal to number of latitude points in each slice).
nDG = nY2; % still weird and ask
...
% Grid information
gridVarList = { 'lat', 'lon', 'ifXY', 'fldStr', 'nD', 'nG', 'nDG' };
if Opts.ifWrite
